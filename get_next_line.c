...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/12/07 10:15:32 by ghaas             #+#    #+#             */
/*   Updated: 2018/01/08 18:57:50 by ghaas            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "get_next_line.h"

t_fd			*search_fd(int fd, t_fd **liste)
{
...
int				read_tmp(t_fd *liste, char **line)
{
	char		*tmp;
	char		*subtmp;
...
int				read_fd(char **line, t_fd *liste)
{
	int			r;
	char		*tmp;
...
int				get_next_line(const int fd, char **line)
{
	...
	if (!(*line = ft_strdup("")))
		return (-1);
	if (!(new_fd = search_fd(fd, &liste)))
		return (-1);
	if (read_tmp(new_fd, line))
		return (*line == NULL ? -1 : 1);
	return (read_fd(line, new_fd));
}
